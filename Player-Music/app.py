...
ALBUM_COVER_IMAGE_PATH = 'Images\\vito-ysl.png'
...
player_info = [
    [sg.Text('Player de Música Feito por vito.ysl',
...
window = sg.Window('Vito-ysl', layout=main, size=(
    480, 730), background_color='black', finalize=True, grab_anywhere=True, resizable=False,)

directory = sg.popup_get_folder('Selecione a sua pasta de música')
...
def update_song_display():
    ...
    window['currently_playing'].update(
        f'Tocando: {os.path.basename(songs_in_directory[current_song_index])}')
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if current_song_index + 1 <= song_count and current_song_index > 0:
            ...
            print('Chegou à primeira música')
